...
public:
  ...
  void finish(int r)
  {
    for (std::list<Session*>::iterator i = projected_sessions.begin();
      ...
      sessionmap->mark_dirty(*i);
    }

    sessionmap->save(on_saved);
  }
};
    ...
    MDSInternalContextBase *on_saved)
{
  ...
  //  * A version that's currently dirty (i.e. `version`)
  //  * A projected version ahead of original_projected (we have some
  //    mark_dirty'ing of our own to do)
  //  * A projected version <= original_projected (we just need
